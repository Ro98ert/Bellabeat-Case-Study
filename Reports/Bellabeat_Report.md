...
# Daily Level
daily_activity <- read_csv(file.path(data_path, "daily_activity.csv")) %>% clean_names()
minute_sleep   <- read_csv(file.path(data_path, "minute_sleep.csv"))   %>% clean_names()
weight_log     <- read_csv(file.path(data_path, "weight_logs.csv"))    %>% clean_names()

# Hourly Data
hourly_steps       <- read_csv(file.path(data_path, "hourly_steps.csv")) %>% clean_names()
hourly_calories    <- read_csv(file.path(data_path, "hourly_calories.csv")) %>% clean_names()
hourly_intensities <- read_csv(file.path(data_path, "hourly_intensities.csv")) %>% clean_names()

# Minute Level
minute_steps       <- read_csv(file.path(data_path, "minute_steps.csv")) %>% clean_names()
minute_calories    <- read_csv(file.path(data_path, "minute_calories.csv")) %>% clean_names()
minute_intensities <- read_csv(file.path(data_path, "minute_intensities.csv")) %>% clean_names()
minute_mets        <- read_csv(file.path(data_path, "minute_MET.csv")) %>% clean_names()

# Heartrate Data
heartrate_seconds  <- read_csv(file.path(data_path, "heartrate_seconds.csv")) %>% clean_names()
...
```{r SQL_clean_daily_activity, eval=FALSE, include=TRUE}
CLEANING SAMPLE: DAILY ACTIVITY & WEIGHT
-- Daily Activity: Remove non-wear days
...
SELECT
  ...
  -- [Additional columns truncated for brevity]
FROM `bellabeat_raw.daily_activity`
WHERE Calories > 0 AND NOT (TotalSteps = 0 AND VeryActiveMinutes > 0);


-- Weight Log: Drop 'Fat' column due to nulls
CREATE OR REPLACE TABLE `bellabeat_clean.weight_logs` AS
SELECT DISTINCT
  Id AS id,
...
FROM `bellabeat_raw.weight_logs`;
```
...
-- Example: Hourly Steps (Trimming whitespace issues)
